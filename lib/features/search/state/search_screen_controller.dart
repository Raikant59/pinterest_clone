...
class SearchScreenController extends Notifier<SearchScreenState> {
  ...
  Future<void> loadData() async {
    if (state.isInitialLoading || state.boardItems.isNotEmpty || state.collageSections.isNotEmpty) {
      return;
    }
    ...
    try {
      final items = await ref.read(searchRepositoryProvider).fetchSearchFeed();
      final collages =
      await ref.read(searchRepositoryProvider).fetchCollageSections();
